...
class MovimientosController extends AppController
{
    ...
    {
           if (in_array($this->request->getParam('action'), ['index','view','editAdmin','indexAdmin','addAdmin','deleteAdmin','indexSearchAdmin','consumo'])) {
       
          
						$tiene= $this->tienepermiso('movimientos',$this->request->getParam('action'), $this->request->getSession()->read('Auth.role'));
						if (!$tiene)
    ...
    public function index()
    {
        ...
        $movimientos = $this->paginate($this->Movimientos->find("all")->where(['cliente_id'=> $this->request->getSession()->read('Auth.cliente_id')])->order(['Movimientos.fecha' => 'DESC']));
    ...
    public function consumo()
    {
        ...
        $movimientos = $this->paginate($this->Movimientos->find("all")->where(['cliente_id'=> $this->request->getSession()->read('Auth.cliente_id'),'tipo'=>1])->order(['Movimientos.fecha' => 'DESC']));
