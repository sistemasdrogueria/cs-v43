...
class CanjesController extends AppController
{
	...
	{
		if (in_array($this->request->getParam('action'), ['index', 'add', 'view', 'indexAdmin', 'addAdmin', 'deleteAdmin', 'indexSearch_admin', 'confirm', 'sendmail', 'viewAdmin'])) {


			$tiene = $this->tienepermiso('canjes', $this->request->getParam('action'), $this->request->getSession()->read('Auth.role'));
			if (!$tiene)
	...
	public function index()
	{
		...
		$canjes = $this->paginate($this->Canjes->find()->where(['cliente_id' => $this->request->getSession()->read('Auth.cliente_id')]));
	...
	public function viewAdmin($id = null)
	{
		...
		//$cliente_id=$this->request->getSession()->read('Auth.cliente_id');
	...
	*/
		$this->viewBuilder()->setLayout('store');
		$canje = $this->Canjes->newEmptyEntity();
		if ($this->request->is('post')) {
			$canje = $this->Canjes->patchEntity($canje, $this->request->getData());
			$puntos_disponibles = $this->request->getSession()->read('User.puntos');
			$puntos_canje = $canje->cantidad_puntos;

			if ($this->request->getSession()->read('User.habilitado') == 0) {
			...
			if (($puntos_canje <= $puntos_disponibles) && ($beneficio->cantidad_disponible > 0)) {
				//punto en template
				$this->request->getSession()->write('User.puntos', $puntos_disponibles - $puntos_canje);
				...
				if ($this->Puntos->save($punto)) {
					...
					if ($this->Beneficios->save($beneficio)) {
						if ($this->Canjes->save($canje)) {
							...
							$this->sendConfirmacionCanje($beneficio, $canje, $this->request->getSession()->read('Auth.cliente_id'));
	...
	function recorrermovimientos($cantidad = null)
	{
		...
		$movimientos = $this->Movimientos->find()->where([
			'cliente_id' => $this->request->getSession()->read('Auth.cliente_id'),
	...
	function sendConfirmacionCanje($beneficio, $canje, $cliente_id)
	{
		...
		try {
			...
			->setReplyTo(['comunidadsur@drogueriasur.com.ar'])
				->setEmailFormat('html')
				->setTo([$para, $para2])
				->setFrom(['comunidadsur@drogueriasur.com.ar' => 'comunidadsur.com.ar'])
				->setSubject('Resumen de tu canje - ' . $beneficio['nombre'])
				->addBcc([$parabcc1, $parabcc2, $parabcc3])
				->viewBuilder()
				->setTemplate('canje', 'default');
				
			$mailer->setViewVars(['cliente' => $cliente, 'beneficio' => $beneficio, 'canje' => $canje]);
			$mailer->deliver();
