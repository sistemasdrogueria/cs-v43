...
class PuntosController extends AppController
{
    ...
    {
           if (in_array($this->request->getParam('action'), ['index','view','editAdmin','addAdmin','deleteAdmin','indexAdmin'])) {
       
          
						$tiene= $this->tienepermiso('puntos',$this->request->getParam('action'), $this->request->getSession()->read('Auth.role'));
						if (!$tiene)
    ...
    public function index()
    {
		...
		$movimientos = $this->Movimientos->find()->where(['cliente_id'=>$this->request->getSession()->read('Auth.cliente_id'),
		...
		$result = $this->Equivalencias->find('all')
				...
				->where(['c.id'=>$this->request->getSession()->read('Auth.cliente_id')])
				->first([]);
    ...
    public function add()
    {
        $punto = $this->Puntos->newEmptyEntity();
        if ($this->request->is('post')) {
