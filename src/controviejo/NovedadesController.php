...
class NovedadesController extends AppController
{
    ...
    {
		if (in_array($this->request->getParam('action'), ['edit_admin', 'delete_admin','index','add_admin','index_admin','noticia','terminos_condiciones'])) {
       
			$tiene= $this->tienepermiso('novedades',$this->request->getParam('action'),$this->request->getSession()->read('Auth.User.role'));
			if (!$tiene)
