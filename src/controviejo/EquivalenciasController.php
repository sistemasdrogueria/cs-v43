...
class EquivalenciasController extends AppController
{
    ...
    {
           if (in_array($this->request->getParam('action'), ['edit_admin','add_admin','delete_admin','index_admin'])) {
       
          
						$tiene= $this->tienepermiso('equivalencias',$this->request->getParam('action'), $this->request->getSession()->read('Auth.User.role'));
						if (!$tiene)
    ...
    {
        $equivalencia = $this->Equivalencias->newEntity();
