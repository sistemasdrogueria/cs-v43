...
class BeneficiosController extends AppController
{
    ...
    {
		if (in_array($this->request->getParam('action'), ['edit_admin', 'delete_admin','index','add_admin','index_admin','search'])) {
       
			$tiene= $this->tienepermiso('beneficios',$this->request->getParam('action'),$this->request->getSession()->read('Auth.User.role'));
			if (!$tiene)
		...
		if ($this->request->is('post')) {
			...
			else
			{
				...
				{					
					if ($categoria_id==2)
					{
						$puntos =$this->request->getSession()->read('Auth.User.puntos');
    ...
    {
		$this->viewBuilder()->setLayout('admin');
        $beneficio = $this->Beneficios->newEntity();
        if ($this->request->is('post')) {
